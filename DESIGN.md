...
- **Details popup**: when a patch is in effect, a `当前单词已纠错` button appears above the source groups. It opens an accessible modal listing each changed field as 原始 → 纠错内容 with the origin level (我的纠错 / 系统纠错); it is a real dialog (focus trap, Escape, focus restore), not an inline expansion.
